...
type PortraitLightboxProps = {
  ...
  imageSrc?: string;
  subtitle?: string;
  /** Department / constituency represented in the House. */
  department?: string;
  /** Academic year represented, e.g. 2025-26. */
...
export function PortraitLightbox({
  ...
  const hasMeta = Boolean(department || yearRepresented);
  ...
  return (
    <>
      ...
      >
        <div
          className="relative flex max-h-[min(90vh,720px)] w-full max-w-md flex-col items-center rounded-2xl border border-border bg-card p-6 shadow-2xl sm:p-8"
        ...
        >
          ...
          <div className="relative mt-2 size-56 overflow-hidden rounded-full ring-4 ring-[var(--institutional-gold)]/35 ring-offset-4 ring-offset-card sm:size-64">
            {imageSrc ? (
              ...
              <div className="flex size-full items-center justify-center bg-primary/10 text-3xl font-semibold tracking-tight text-primary sm:text-4xl">
                {formatPersonInitials(name)}
              </div>
            )}
          </div>
          ...
          {hasMeta ? (
            <dl className="mt-5 w-full max-w-xs space-y-2.5 border-t border-border pt-5 text-center">
              {department ? (
                <div>
